...
subject_scores = df.groupby('anonymized_subject_id')['loes-score'].agg(['mean', 'std', 'count'])
...
bars = ax1.barh(range(len(subject_scores)), subject_scores['mean'], 
                xerr=subject_scores['std'], color=colors, alpha=0.8,
                edgecolor='white', linewidth=1.5)

ax1.set_yticks(range(len(subject_scores)))
ax1.set_yticklabels([s.replace('subject-', 'S') for s in subject_scores.index], fontsize=9)
...
for i, (bar, val) in enumerate(zip(bars, subject_scores['mean'])):
    ax1.text(val + 0.1, bar.get_y() + bar.get_height()/2, 
             f'{val:.1f}', va='center', fontsize=9, color='cyan')

score_bins = [0, 5, 10, 15, 20, 25, 30, 35]
score_labels = ['0-5', '5-10', '10-15', '15-20', '20-25', '25-30', '30-35']
df['score_category'] = pd.cut(df['loes-score'], bins=score_bins, labels=score_labels, include_lowest=True)
...
subject_progression = df.pivot_table(index='anonymized_session_id', 
                                    columns='anonymized_subject_id', 
                                    values='loes-score')

sample_subjects = df['anonymized_subject_id'].value_counts().head(8).index

for subject in sample_subjects:
    subject_data = df[df['anonymized_subject_id'] == subject].sort_values('anonymized_session_id')
    if len(subject_data) > 1:
        sessions = range(len(subject_data))
        scores = subject_data['loes-score'].values
        ax3.plot(sessions, scores, marker='o', linewidth=2, markersize=6,
                label=subject.replace('subject-', 'S'), alpha=0.8)
...
if score_changes:
    n, bins, patches = ax4.hist(score_changes, bins=15, alpha=0.7, edgecolor='white', linewidth=1.5)
    ...
    ax4.axvline(np.mean(score_changes), color='gold', linestyle='--', linewidth=2, 
    ...
    ax4.text(0.98, 0.98, f'Improved: {improved}\nWorsened: {worsened}\nUnchanged: {unchanged}',
...
ax4.set_title('Score Change Distribution (Multi-Session Subjects)', fontsize=13, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('loes_analysis_dashboard.png', dpi=150, facecolor='#0a0a0a', edgecolor='none')
plt.show()
...
print(f"  • Score variance indicates {('high' if df['loes-score'].std() > 5 else 'moderate')} variability")
print(f"  • {'Most' if (df['loes-score'] < 10).sum() > len(df)/2 else 'Many'} scores are in the lower range")