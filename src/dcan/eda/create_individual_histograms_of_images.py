...
def get_file_identifiers(file: Path) -> tuple[str, str, str]:
    ...
    try:
        ...
        run_id = next(
            (part for part in parts if part.startswith("run-")), "run-01"
...
def mask_in_matter_data(
    img_dir: Path, subject_id: str, session_id: str, mask_path: Path
) -> np.ndarray:
    """
    Apply a mask to the subject's brain image and return the filtered voxel intensities.
    """
    brain_path = (
        img_dir / f"{subject_id}_{session_id}_space-MNI_brain_mprage_RAVEL.nii.gz"
    )

    if not brain_path.exists():
        print(
            f"Missing brain image: {brain_path}. Skipping subject {subject_id}-{session_id}."
        )
        return None

    if not mask_path.exists():
        print(
            f"Missing mask file: {mask_path}. Skipping subject {subject_id}-{session_id}."
        )
        return None
...
def plot_voxel_intensity_histogram(
    ...
    if gm_data is None or wm_data is None:
        print(
            f"Skipping histogram generation for {subject_id}-{session_id} due to missing data."
        )
        return
    ...
    sns.histplot(
        gm_data.flatten(), label="Gray Matter (GM)", kde=True, color="blue", alpha=0.6
    )
    sns.histplot(
        wm_data.flatten(),
        label="White Matter (WM)",
        kde=True,
        color="orange",
        alpha=0.6,
    )
    ...
    hist_path = (
        hist_dir
        / f"masked-{subject_id}_{session_id}_space-MNI_brain_voxel_histogram.png"
    )
...
def process_directory(img_dir: Path, hist_dir: Path, wm_mask: Path, gm_mask: Path):
    ...
    for file in tqdm.tqdm(files, desc="Processing Subjects"):
        try:
            ...
            plot_voxel_intensity_histogram(
                subject_id, session_id, img_dir, hist_dir, wm_mask, gm_mask
            )
...
def main(img_dir: str, hist_dir: str, wm_mask: str, gm_mask: str):
    ...
    if not wm_mask.exists() or not gm_mask.exists():
        print(
            f"Error: One or both mask files are missing.\nWM Mask: {wm_mask}\nGM Mask: {gm_mask}"
        )
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print(
            "Usage: python script.py <image_directory> <histogram_output_directory> <wm_mask> <gm_mask>"
        )
        sys.exit(1)
