...
class Dense3DCNN(nn.Module):
    ...
    def __init__(self, growth_rate=16, block_config=(3, 3, 3), debug=False):
        ...
        self.features = nn.Sequential(
            nn.Conv3d(
                1, growth_rate * 2, kernel_size=7, stride=2, padding=3, bias=False
            ),
...
class EfficientNet3D(nn.Module):
    def __init__(
        self, width_multiplier=1.0, depth_multiplier=1.0, dropout_rate=0.2, debug=False
    ):
        ...
        for t, c, n, s, k in settings:
            ...
            self.blocks.append(
                MBConvBlock3D(input_channels, output_channels, k, s, expand_ratio=t)
            )
