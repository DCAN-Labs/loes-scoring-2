...
class DataHandler:
    ...
    def init_dl(self, folder, subjects, is_val_set: bool = False):
        ...
        if valid_subjects == 0:
            ...
            log.warning(
                f"Only {valid_subjects} out of {len(subjects)} subjects exist in the dataset"
            )
        ...
        try:
            if (
                not is_val_set
                and hasattr(self, "augment_minority")
                and self.augment_minority
            ):
...
class SimpleMRIModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.initialized:
            ...
            if self.debug:
                log.debug(
                    f"Model automatically initialized with input size: {self.input_dim}"
                )
...
class Config:
    def __init__(self):
        ...
        self.parser.add_argument(
            "--csv-input-file", required=True, help="CSV data file"
        )
        self.parser.add_argument(
            "--csv-output-file", help="CSV output file for predictions"
        )
        self.parser.add_argument(
            "--features", required=True, nargs="+", help="Feature column names"
        )
        ...
        self.parser.add_argument(
            "--num-workers", default=4, type=int, help="Number of worker processes"
        )
        self.parser.add_argument(
            "--split-ratio",
            default=0.8,
        ...
        self.parser.add_argument(
            "--weight-decay", default=0.0001, type=float, help="L2 regularization"
        )
        self.parser.add_argument(
            "--model-type",
            default="conv",
        ...
        # Output and tracking
        self.parser.add_argument(
            "--tb-prefix", default="logistic_regression", help="Tensorboard data prefix"
        )
        ...
        self.parser.add_argument(
            "--comment", default="", help="Comment for Tensorboard run"
        )
        self.parser.add_argument(
            "--normalize-features", action="store_true", help="Normalize input features"
        )

        # Evaluation
        self.parser.add_argument(
            "--threshold", default=0.5, type=float, help="Classification threshold"
        )
    ...
    def validate_args(self, args):
        ...
        if args.num_workers < 0:
            raise ValueError(
                f"Number of workers must be non-negative, got {args.num_workers}"
            )

        if not (0 < args.split_ratio < 1):
            raise ValueError(
                f"Split ratio must be between 0 and 1, got {args.split_ratio}"
            )

        if args.weight_decay < 0:
            raise ValueError(
                f"Weight decay must be non-negative, got {args.weight_decay}"
            )

        if not (0 <= args.threshold <= 1):
            raise ValueError(
                f"Classification threshold must be between 0 and 1, got {args.threshold}"
            )
        ...
        if (
            args.model_type in ["resnet3d", "dense3d", "efficientnet3d"]
            and not args.folder
        ):
            raise ValueError(
                f"Model type '{args.model_type}' requires MRI data folder (--folder)"
            )

        # Optimizer/scheduler compatibility
        if args.scheduler == "onecycle" and args.optimizer.lower() != "adam":
            log.warning(
                f"OneCycle scheduler works best with Adam optimizer, but {args.optimizer} was specified"
            )

        # Augmentation parameter checks
        if args.augment_minority and args.num_augmentations <= 0:
            raise ValueError(
                "Number of augmentations must be positive when augment_minority is enabled"
            )

        # Warning for potentially problematic configurations
        if args.num_workers > 8:
            log.warning(
                f"High number of workers ({args.num_workers}) may cause memory issues"
            )
        ...
        with torch.no_grad():
            for batch_tup in val_dl:
                if len(batch_tup) == 4:
                    ...
                    label = [
                        1.0 if l_t.item() > self.threshold else 0.0 for l_t in label_t
                    ]
            ...
            for threshold in threshold_candidates:
                ...
                if f1_score > best_f1:
                    ...
                    best_metrics["pauc"] = (
                        pauc_score  # pAUC is the same regardless of threshold
                    )
        ...
        if len(batch_tup) == 4:
            ...
            if label_t is None:
                ...
                label_t = (
                    batch_tup[1].float()
                    if len(batch_tup) > 1
                    else torch.zeros_like(input_t[:, 0, 0, 0])
                )
...
class TensorBoardLogger:
    ...
    def log_metrics(self, mode_str, epoch, metrics, sample_count):
        ...
        # Log basic metrics
        writer.add_scalar("loss/all", metrics[METRICS_LOSS_NDX].mean(), sample_count)
        ...
        try:
            ...
            writer.add_scalar("metrics/accuracy", acc, sample_count)
            writer.add_scalar("metrics/precision", prec, sample_count)
            writer.add_scalar("metrics/recall", rec, sample_count)
            writer.add_scalar("metrics/f1", f1, sample_count)
            ...
            if len(np.unique(y_true)) > 1:
                ...
                writer.add_scalar("metrics/auc", auc_score, sample_count)
                writer.add_scalar("metrics/pauc", pauc_score, sample_count)
        except Exception as e:
...
class LogisticRegressionApp:
    def __init__(self, sys_argv=None, input_df=None, output_df=None):
        ...
        if self.config.use_train_validation_cols:
            ...
            self.train_subjects = list(
                set(training_rows["anonymized_subject_id"].tolist())
            )

            validation_rows = self.input_df[self.input_df["validation"] == 1]
            self.val_subjects = list(
                set(validation_rows["anonymized_subject_id"].tolist())
            )
        else:
        ...
        self.val_dl = self.data_handler.init_dl(
            self.folder, self.val_subjects, is_val_set=True
        )
    ...
    def _standard_train(self):
        ...
        if len(self.val_dl.dataset) == 0:
            raise ValueError(
                "Validation dataset is empty. Cannot proceed with training."
            )

        try:
            # Initialize the trainer with model, optimizer, and device
            trainer = LogisticRegressionTrainer(
                self.model, self.optimizer, self.device, threshold=self.config.threshold
            )
            ...
            for epoch in range(1, self.config.epochs + 1):
                ...
                self.tb_logger.log_metrics(
                    "trn", epoch, trn_metrics, trainer.total_samples
                )
                self.tb_logger.log_metrics(
                    "val", epoch, val_metrics, trainer.total_samples
                )
            ...
            log.info(
                f"Training completed. Best validation accuracy: {best_val_acc:.4f} (epoch {best_epoch})"
            )
    ...
    def train_with_threshold_optimization(self, fold_idx=None):
        ...
        for epoch in range(1, self.config.epochs + 1):
            ...
            optimal_threshold, same_pauc, threshold_metrics = (
                trainer.find_optimal_threshold_for_pauc(self.val_dl)
            )

            # Verify that pAUC is the same (as it should be)
            assert abs(current_pauc - same_pauc) < 1e-6, (
                f"pAUC should be threshold-independent! {current_pauc} vs {same_pauc}"
            )
            ...
            log.info(
                f"  F1 Score at optimal threshold: {threshold_metrics.get('f1', 0):.4f}"
            )
            log.info(
                f"  Accuracy at optimal threshold: {threshold_metrics.get('accuracy', 0):.4f}"
            )
            ...
            epoch_metrics["paucs"].append(
                current_pauc
            )  # This should vary across epochs as model improves
        ...
        if self.config.model_save_location:
            ...
            log.info(
                f"Model and threshold info saved to {save_path} and {threshold_path}"
            )

        log.info(
            f"Training completed. Best pAUC: {best_pauc:.4f} at epoch {best_epoch}"
        )
    ...
    def _setup_scheduler(self):
        ...
        if self.config.scheduler == "step":
            ...
            log.info("Step scheduler with step_size=30, gamma=0.1")

        elif self.config.scheduler == "cosine":
            self.scheduler = CosineAnnealingLR(
                self.optimizer, T_max=self.config.epochs, eta_min=0
            )
            ...
            log.info(
                f"OneCycle scheduler with total_steps={total_steps}, pct_start=0.3"
            )
            ...
            log.info("ReduceLROnPlateau scheduler with factor=0.1, patience=10")
            ...
            if model_type == "simple":
                ...
                try:
                    ...
                    self.model = get_mri_logistic_regression_model(
                        model_type="conv", debug=debug_mode
                    )
                except ImportError as e:
                    log.error(
                        f"Failed to import get_mri_logistic_regression_model: {e}"
                    )
                ...
                try:
                    ...
                    self.model = get_advanced_mri_model(
                        model_type=model_type, debug=debug_mode
                    )
                except ImportError as e:
                    log.error(f"Failed to import advanced model modules: {e}")
                    raise ValueError(
                        f"Advanced model '{model_type}' requires additional dependencies that are not available"
                    )
        ...
        try:
            ...
            if not os.path.exists(self.config.csv_input_file):
                raise FileNotFoundError(
                    f"Input file not found: {self.config.csv_input_file}"
                )
            ...
            missing_features = [
                f for f in self.config.features if f not in self.input_df.columns
            ]
            if missing_features:
                raise ValueError(
            ...
            if self.config.target not in self.input_df.columns:
                raise ValueError(
                    f"Target column '{self.config.target}' not found in input file"
                )
            ...
            log.info(
                f"Dataset loaded successfully: {self.input_df.shape[0]} rows, {self.input_df.shape[1]} columns"
            )
            log.info(f"Features: {self.config.features}")
            log.info(f"Target: {self.config.target}")
            log.info(
                f"Class distribution: {self.input_df[self.config.target].value_counts().to_dict()}"
            )
    ...
    def run_cross_validation(self, k=5):
        ...
        for subject in all_subjects:
            subject_rows = self.input_df[
                self.input_df["anonymized_subject_id"] == subject
            ]
        ...
        log.info(
            f"Total subjects: {len(all_subjects)} ({len(subjects_with_ald)} with ALD, {len(subjects_without_ald)} without ALD)"
        )
        ...
        for fold in range(k):
            ...
            train_subjects_ald = [
                s
                for i, fold_subjects in enumerate(ald_folds)
                for s in fold_subjects
                if i != fold
            ]
            train_subjects_no_ald = [
                s
                for i, fold_subjects in enumerate(no_ald_folds)
                for s in fold_subjects
                if i != fold
            ]
            ...
            log.info(
                f"Train set: {len(self.train_subjects)} subjects ({len(train_subjects_ald)} with ALD, {len(train_subjects_no_ald)} without ALD)"
            )
            log.info(
                f"Val set: {len(self.val_subjects)} subjects ({len(val_subjects_ald)} with ALD, {len(val_subjects_no_ald)} without ALD)"
            )
            ...
            if fold_results.get("pAUC", 0) > best_val_pauc:
                ...
                fold_model_path = (
                    f"{os.path.splitext(best_model_path)[0]}_fold{fold + 1}.pt"
                )
    ...
    def generate_summary_statistics(self):
        ...
        train_rows = self.input_df[
            self.input_df["anonymized_subject_id"].isin(self.train_subjects)
        ]
        val_rows = self.input_df[
            self.input_df["anonymized_subject_id"].isin(self.val_subjects)
        ]
        ...
        log.info(
            f"Subjects: {total_subjects} total ({train_subjects_count} training, {val_subjects_count} validation)"
        )
        log.info(
            f"Scans: {total_scans} total ({train_scans} training, {val_scans} validation)"
        )
        ...
        try:
            ...
            log.info("\nCLASS DISTRIBUTION:")
            log.info(
                f"Training: {train_positive} positive ({train_positive / train_scans * 100:.1f}%), "
                + f"{train_negative} negative ({train_negative / train_scans * 100:.1f}%)"
            )
            log.info(
                f"Validation: {val_positive} positive ({val_positive / val_scans * 100:.1f}%), "
        ...
        try:
            ...
            with torch.no_grad():
                for batch_tup in self.val_dl:
                    input_t, label_t, _, _ = batch_tup
                    label = [
                        1.0 if l_t.item() > self.config.threshold else 0.0
                        for l_t in label_t
                    ]
            ...
            if len(np.unique(val_labels)) > 1:
                ...
                pauc_score = calculate_pauc(
                    val_labels, val_probs, max_fpr=0.1
                )  # ADD pAUC

                log.info(f"AUC: {auc_score:.4f}")
                log.info(
                    f"pAUC (FPR \u2264 0.1): {pauc_score:.4f} \u2b50 PRIMARY METRIC"
                )  # HIGHLIGHT pAUC
    ...
    def plot_roc_curve(self, val_dl=None, save_path=None, fold_idx=None):
        ...
        plt.plot(
            fpr[current_threshold_idx],
            tpr[current_threshold_idx],
            "ro",
            markersize=10,
    ...
    def run_cross_validation_with_combined_roc(self, k=5):
        ...
        for fold in range(k):
            ...
            if fold == 2:  # Fold 3 (0-indexed)
                ...
                train_data = self.input_df[
                    self.input_df["anonymized_subject_id"].isin(self.train_subjects)
                ]
                val_data = self.input_df[
                    self.input_df["anonymized_subject_id"].isin(self.val_subjects)
                ]
                ...
                print(
                    f"Train: {train_pos}/{len(train_data)} positive ({train_pos / len(train_data) * 100:.1f}%)"
                )
                print(
                    f"Val: {val_pos}/{len(val_data)} positive ({val_pos / len(val_data) * 100:.1f}%)"
                )
                ...
                print(
                    f"Missing values - Train: {train_data.isnull().sum().sum()}, Val: {val_data.isnull().sum().sum()}"
                )
            ...
            if fold == 2:  # After training fold 3
                ...
                if val_probs.std() < 0.01:
                    print(
                        "\u26a0\ufe0f  Model predictions have very low variance - possible training failure"
                    )
                ...
                if len(np.unique(val_labels)) > 1:
                    ...
                    if pos_probs.mean() < neg_probs.mean():
                        print(
                            "\u26a0\ufe0f  Model predicts LOWER probabilities for positive class - inverted learning!"
                        )
            ...
            if len(np.unique(fold_y_true)) < 2:
                log.warning(
                    f"Fold {fold + 1} has only one class, skipping ROC calculation"
                )
                continue
    ...
    def _plot_combined_roc_curves(self, fold_roc_data, mean_fpr, k):
        ...
        plt.title(
            f"{len(fold_roc_data['original_fprs'])}-Fold Cross-Validation ROC Curves",
            fontsize=16,
        )
        ...
        if hasattr(self.config, "plot_location") and self.config.plot_location:
            ...
            if os.path.isdir(self.config.plot_location):
                plot_path = os.path.join(
                    self.config.plot_location, f"combined_roc_cv_{self.time_str}.png"
                )
            else:
                base_path = os.path.splitext(self.config.plot_location)[0]
    ...
    def _plot_overall_roc_curve(self, fold_roc_data):
        ...
        overall_fpr, overall_tpr, overall_thresholds = roc_curve(
            fold_roc_data["y_true_all"], fold_roc_data["y_prob_all"]
        )
        ...
        for threshold in key_thresholds:
            if len(overall_thresholds) > 0:
                ...
                if closest_idx < len(overall_fpr):
                    plt.scatter(
                        overall_fpr[closest_idx],
                        overall_tpr[closest_idx],
                        s=80,
                        alpha=0.8,
    ...
    def _print_cv_summary(self, fold_metrics, fold_aucs):
        ...
        if pauc_values:
            ...
            log.info(
                f"\u2b50 PRIMARY METRIC - pAUC (FPR \u2264 0.1): {mean_pauc:.4f} ± {std_pauc:.4f}"
            )
...
class pAUCMonitor:
    ...
    def update(self, epoch, y_true, y_prob):
        ...
        status = (
            "\U0001f4c8 NEW BEST"
            if is_best
            else f"\U0001f4c9 No improvement ({self.epochs_without_improvement}/{self.patience})"
        )
...
if __name__ == "__main__":
    try:
        ...
        if log_reg_app.config.DEBUG:
            ...
            fold_metrics, fold_roc_data = (
                log_reg_app.run_cross_validation_with_combined_roc(k=5)
            )
        else:
