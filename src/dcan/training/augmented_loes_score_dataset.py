...
class AugmentedLoesScoreDataset(LoesScoreDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.augment_minority = (
            augment_minority and not is_val_set_bool
        ...
        if self.augment_minority:
            ...
            log.info(
                f"Found {len(minority_candidates)} minority class samples in dataset"
            )
            ...
            for candidate in minority_candidates:
                try:
                    ...
                    log.error(
                        f"Failed to create augmented samples for candidate {candidate.subject_str}: {e}"
                    )
                    continue

            log.info(
                f"Created {len(self.augmented_candidates)} augmented minority samples"
            )
    ...
    def __getitem__(self, idx):
        ...
        # Return augmented sample
            ...
            # Create label
            cald_develops = torch.tensor(
                0.0, dtype=torch.float32
            )  # Always 0 for minority class

            return (
                aug_tensor,
                cald_develops,
                aug_candidate.subject_str,
                aug_candidate.session_str,
            )
