...
class MRIAugmenter:
    ...
    def random_flip_lr(self, image, prob=0.5):
        ...
        if np.random.random() < prob:
            flipped = np.flip(
                image, axis=-3
            )  # Assuming shape [C, D, H, W], flip D axis
            return flipped.copy()  # Make a copy to avoid negative strides
        return image

    def random_affine_transform(
        self, image, scale_range=(0.9, 1.1), rotation_degrees=10
    ):
        ...
        if scale != 1.0:
            ...
            if scale > 1.0:  # Image got bigger, crop center
                ...
                start_indices = [
                    (s - o) // 2 for s, o in zip(image_3d.shape, original_size)
                ]
                end_indices = [
                    start + o for start, o in zip(start_indices, original_size)
                ]
                image_3d = image_3d[
                ...
                original_size = original_shape[-3:] if process_3d else image.shape
                pad_widths = [
                    (max(0, (o - s) // 2), max(0, o - s - (o - s) // 2))
                    for s, o in zip(image_3d.shape, original_size)
                ]
                image_3d = np.pad(
                    image_3d, pad_widths, mode="constant", constant_values=0
                )
        ...
        if abs(angle) > 0.1:  # Only rotate if angle is significant
            ...
            image_3d = ndimage.rotate(
                image_3d, angle, axes=axes, reshape=False, order=1
            )
    ...
    def random_elastic_deformation(self, image, max_displacement=5):
        ...
        displacement_fields = []
        for dim in range(3):
            coarse_field = np.random.uniform(
                -max_displacement, max_displacement, coarse_shape
            )
            # Smooth the field
            coarse_field = ndimage.gaussian_filter(coarse_field, sigma=1.0)
            # Interpolate to full resolution
            full_field = ndimage.zoom(
                coarse_field, [s / c for s, c in zip(shape, coarse_shape)], order=1
            )
        ...
        deformed = ndimage.map_coordinates(
            image_3d, new_coords, order=1, mode="nearest"
        )
        ...
        for _ in range(self.num_augmentations):
            ...
            for transform_func in selected_functions:
                try:
                    ...
                    if (
                        isinstance(augmented_img, np.ndarray)
                        and not augmented_img.flags["C_CONTIGUOUS"]
                    ):
            ...
            if isinstance(augmented_img, np.ndarray):
                if not augmented_img.flags["C_CONTIGUOUS"] or any(
                    s < 0 for s in augmented_img.strides
                ):
                    augmented_img = np.ascontiguousarray(augmented_img)
