...
This module provides functionality for predicting Loes scores from brain MRI scans
...
def predict(row, data_folder):
    ...
    mprage_path = (
        f"{data_folder}/{subject}_{session}_space-MNI_brain_mprage_RAVEL.nii.gz"
    )
...
def get_predicted_value(row, subjects, sessions, predict_vals):
    ...
    for subject, session, predict_val in zipped_data:
        if (
            row["anonymized_subject_id"] == subject
            and row["anonymized_session_id"] == session
        ):
            return predict_val
    return np.nan
...
def add_predicted_values(subjects, sessions, predict_vals, input_csv_location):
    ...
    output_df["predicted_loes_score"] = output_df.apply(
        get_predicted_value, axis=1, args=(subjects, sessions, predict_vals)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_pattern",
        default="*_RAVEL.nii.gz",
        help="The pattern of the NIFTI files, such as _RAVEL.nii.gz",
    )
    parser.add_argument(
        "--device",
        default="gpu",
    ...
    parser.add_argument(
        ...
        help="Choose the model architecture (resnet or alexnet)",
    )
    parser.add_argument(
        "--validation_csv_file_path",
        help="For validation of known Loes scores, the path to the CSV containing those scores.",
    )
    parser.add_argument(
